			...
			if(a[i][i] != 0){
				...
				for (int j = 0; j < numLinhas; j++){
					if(j != i){
						...
						//printf("Valor do a(i,j) = %d valor do x2 = %d\n",a[i][j],vetorsolucao[j][0]);
				...
				if(numLinhas == 2){
					...
					vetorAuxiliar[0][0] = matrizParaCalcular[0][0];
					vetorAuxiliar[0][1] = matrizParaCalcular[0][1];
					//printf("MARTRIZPARACALCULAR %d - cont %d\n",matrizParaCalcular[i][0],cont);
						//printf("MARTRIZPARACALCULAR %d - cont %d\n",matrizParaCalcular[i][1],cont);
					...
					vetorsolucao[i][0] = dividirFracao(b[i],a[i][i],1,1,1);
					vetorsolucao[i][1] = dividirFracao(b[i],a[i][i],1,1,2);
